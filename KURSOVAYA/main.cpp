...
void toArray(list *l, list *r[4000]);
int compare(char *, char *);
void binSearch(char *x, list *l[4000], queue *&turn);
...
int main() {
    ...
    while (true) {
        ...
        enter = 0;
        SetConsoleCP(866);
        cout << "\t1. Просмотр базы данных (< - назад, esc - выйти)" << endl;
        ...
        cout << "\t5. Статистика по кодированию" << endl;
        cout << "\tEsc. Выход " << endl;
        ...
        switch (enter) {
            ...
            case 51:
                ...
                char X[3];
                SetConsoleCP(866);
                cout << "Введите ключ поиска (три буквы искомой публикации): ";
                cin >> X;
                toArray(sbase, mas_sbase);
                binSearch(X, mas_sbase, stack);
            ...
            case 52:
                ...
                cout << "Посмотреть дерево (1 - да, другая клавиша - нет) ";
                cin >> x;
                if (x == 1) {
                    print_tree(root);
                }
                SetConsoleCP(866);
                cout << "Введите сумму искомого вклада: " << endl;
                cin >> x;
                tree_search(root, x);
                break;
            case 53:
...
void print_base(list *l) {
    ...
    int c = 0;
    cout << "1 - вывод по 20 (управление 1 - предыдущая страница, 2 - следующая страница)" << endl
         << "2 - вывод всего списка" << endl;
    ...
    while (flag) {
        ...
        if (broke == 0) {
            ...
            } else if (c == 27) { ///Пользоавтель нажал на кнопку = ESC
                flag = 0;
                return;
            }
        } else {
...
void binSearch(char *x, list **l, queue *&turn) {
    int L = 1;
    int R = N;
    int m, h, t;
    int check;
    ...
    while (L < R) {
        m = (L + R) / 2;
        check = compare(l[m]->data->depositor, x);
        if (check == 1) {
            L = m + 1;
        } else {
            if (check == -1) {
...
void add(vertex *&point, record *x) {
    ...
    while (*p) {
        if ((*x).deposit < (*p)->fact->data->deposit) {
            p = &((*p)->left);
        } else if ((*x).deposit > (*p)->fact->data->deposit) {
            p = &((*p)->right);
        } else if ((*x).deposit == (*p)->fact->data->deposit) {
            tmp = (*p)->fact;
            while (tmp->next != NULL) {
...
void tree_search(vertex *&p, unsigned short int x) {
    ...
    while (tmp != NULL) {
        if (tmp->fact->data->deposit == x) {
            print_base(tmp->fact);
            return;
        } else if (tmp->fact->data->deposit > x) {
            tmp = tmp->left;
        } else if (tmp->fact->data->deposit < x) {
            tmp = tmp->right;
        }
    }
    cout << "Вклад не найден" << endl;
}
